...
const AmountStep = ({ data, updateData, userBalance, onNext, onPrev }: AmountStepProps) => {
  ...
  const getFee = (amount: number) => {
    if (data.recipientVerified) return 0; // No fee for verified users
    if (amount <= 1000) return 5;
    if (amount <= 5000) return 10;
    return 15;
  };

  const fee = getFee(data.amount || 0);
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600 text-sm">
          Sending to <span className="font-semibold text-blue-600">{data.recipientName}</span>
        </p>
      </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1">
            <div className="flex items-center space-x-2">
              <h3 className="font-medium text-gray-900">{data.recipientName}</h3>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="space-y-2">
            ...
            <div className="border-t border-blue-200 pt-2">
              <div className="flex justify-between items-center">
                <span className="font-semibold text-blue-900">Total Deduction</span>
                <span className="font-bold text-blue-900">৳{totalDeduction.toLocaleString()}</span>
              </div>
            </div>
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h4 className={`text-sm font-medium ${
              ...
              {data.recipientVerified ? 'No Fee - Verified User' : 'Service Fee Applied'}
            </h4>
      ...
      <div className="flex space-x-4">
        ...
        <button
          onClick={handleNext}
          disabled={!data.amount || data.amount < 10 || totalDeduction > userBalance}
