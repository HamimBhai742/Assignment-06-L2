...
const ConfirmationStep = ({ data, userBalance, onPrev }: ConfirmationStepProps) => {
  ...
  const getFee = () => {
    if (data.recipientVerified) return 0;
    if ((data.amount || 0) <= 1000) return 5;
    if ((data.amount || 0) <= 5000) return 10;
    return 15;
  };
      ...
      <div className="text-center space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-600">
            ৳{data.amount?.toLocaleString()} has been sent to {data.recipientName}
          </p>
        </div>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <div className="text-sm space-y-2">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-gray-900">{data.recipientName}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Purpose:</span>
              <span className="text-gray-900">{data.purpose}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h4 className="text-sm font-medium text-blue-900">Notification Sent</h4>
              <p className="text-xs text-blue-700 mt-1">
                {data.recipientName} has been notified about this transaction via SMS and app notification.
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600 text-sm">Please review and confirm your transaction</p>
      </div>
      ...
      <div className="bg-gray-50 rounded-xl p-6 space-y-4">
        ...
        {data.reference && (
          <div className="flex items-center justify-between">
            ...
            <span className="text-gray-900 text-right max-w-32 truncate">{data.reference}</span>
          </div>
        )}
        ...
        <div className="border-t border-gray-200 pt-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xl font-bold text-blue-600">৳{totalDeduction.toLocaleString()}</span>
          </div>
        </div>
      ...
      <div className={`border rounded-lg p-4 ${
        ...
        <div className="flex items-center space-x-3">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
            ...
            <span className="text-lg">{data.recipientVerified ? '✅' : '⚠️'}</span>
          </div>
          <div className="flex-1">
            <h3 className={`font-medium ${
              data.recipientVerified ? 'text-green-900' : 'text-orange-900'
            }`}>
              {data.recipientVerified ? 'Verified Recipient' : 'Unverified Recipient'}
            </h3>
            <p className={`text-sm ${
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-blue-700 mt-1">
              This transaction is protected with end-to-end encryption. Your PIN is never stored or shared.
            </p>
          </div>
        </div>
      </div>
